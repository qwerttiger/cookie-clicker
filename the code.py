...
def load_save_data(): #load save data
  ...
  gstage=int(file[60])
  gustage=int(file[61])
  pledge=timer(int(file[62]),finishfrenzy) if file[62]!="0" else timer(0,lambda:None)
...
def draw_text(text,pos,size=8,side=True,surface=screen,bg=True): #draw text
  if bg:
    if not side: #if not side
      surface.blit(pygame.font.SysFont("arial",size).render(text,True,black,(254,254,254)),(pos[0]-round(pygame.font.SysFont("arial",size).render(text,True,black).get_width()/2),pos[1]-pygame.font.SysFont("arial",size).render(text,True,black).get_height()/2)) #draw it
    else: #if side
      surface.blit(pygame.font.SysFont("arial",size).render(text,True,black,(254,254,254)),(pos[0],pos[1]-pygame.font.SysFont("arial",size).render(text,True,black).get_height()/2)) #draw it
  else:
    surface.blit(pygame.font.SysFont("arial",size).render(text,True,black),(pos[0],pos[1]-pygame.font.SysFont("arial",size).render(text,True,black).get_height()/2))
def draw_text2(text,pos,size=(180,25)):
  ...
  if x.get_width()<=size[0]: #if its not too wide
    draw_text(text,(pos[0],pos[1]+size[1]//2),22,bg=False) #draw text
  else: #if its to wide
...
def add_cookies(): #add cookies
  global cookies,total_cookies,t,wrinklerslist #global variables
  
  cookies+=cps*decimal(1/10-len(wrinklerslist)/200) #add cookies
  total_cookies+=cps*decimal(1/10-len(wrinklerslist)/200) #add total cookies
  if wrinklers:
    for _,ate in wrinklerslist:
      ate+=cps/10*len(wrinklerslist)
...
class wrath: #class wrath cookie
  def __init__(self): #define init
    self.pos=[random.randint(0,608),random.randint(0,608)] #position
    self.randout=random.choice(["Ruin","Frenzy"]) #random outcome
    self.timestart=time.time() #the time it appeared
  ...
  def effect(self): #define effect
    if self.randout=="Ruin": #if it is lucky
      global cookies #global variables
      
      cookies-=max(min(round(decimal(0.2)*cookies),1200*cps),0)
    
    if self.randout=="Frenzy": #if its frenzy
      frenzy() #then frenzy
  
  def draw(self): #draw itself
    screen.blit(wrath_cookie,self.pos) #draw
    if time.time()>=self.timestart+15: #if has run out of time
      goldens.remove(self) #delete it
...
def pledgefunc(): #pledge function
  ...
  cookies+=sum([popped*1.1 for _,popped in wrinklerslist]) #pop all wrinklers
...
while True: #game loop
  presentclick=bool(sum(pygame.mouse.get_pressed()))
  if wrinklers and len(wrinklerslist)<=9 and random.randint(1,wtime)==1: #if there is a wrinkler spot and a 1/wtime chance
  ...
  try: #try to
    unum=0 #place in upgrades to unlock
    ...
    for _,unlock_cond,name,_,upgrade_id,_,_ in upgrades_to_unlock: #for every upgrade
      if eval(unlock_cond): #if the condition is true
        ...
        upgrades_to_unlock[unum:unum+1]=[] #delete it in upgrades to unlock
      unum+=1 #increment by 1 for the next upgrade
  ...
  if rightpanel=="u": #upgrade right panel
    ...
    for effect,name,icon,upgrade_id2,desc,price in [(a,b,c,d,e,f) for a,_,b,c,d,e,f in upgrades if unlocked_upgrades[d-1]=="1"]: #for every unlocked upgrade
      ...
      if 500+25*((upgrade_id-1)%8)<=pygame.mouse.get_pos()[0]<=525+25*((upgrade_id-1)%8) and 25*((upgrade_id-1)//8)<=pygame.mouse.get_pos()[1]<=25+25*((upgrade_id-1)//8): #if you hover over it
        if not lastclick and presentclick and not bought and cookies>=price and not sum([int(bool(gc.check())) for gc in goldens]): #if you click and you have enough cookies
          clickwrinkler=False
  ...
  if not lastclick and presentclick:
